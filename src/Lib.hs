...
commandPart2 _ _ = error "Unexpected args"
...
sumLists :: [[Int]] -> [Int]
sumLists xs = map (sum . (\i -> map (!! i) xs)) $ range $ length $ head xs

findMostDigit :: Int -> [Int] -> [Int]
findMostDigit _ [] = []
findMostDigit n (x:xs) =
  (if x <= n `div` 2
     then 0
     else 1) :
  findMostDigit n xs
       ...
       if length acc == 1
         then acc
                ...
                (\l ->
                   (l !! n) ==
                   (if most
                      then findMostDigitNPlace
                      else findLeastDigitNPlace)
                     n
                     acc)
                acc)
    xs $
  range $ length $ head xs
